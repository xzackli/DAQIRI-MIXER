/* rx_ula_corr.cu — 4-element UNIFORM LINEAR ARRAY X-engine (the GPU side of the FX correlator).
 * Consumes CHANNELIZED int8 from the F-engine (config_ula.h wire). Per frequency channel it forms the
...
#define NB     ULA_PAYLOAD_BYTES           /* 8192 payload bytes per element heap (interleaved int8 re,im) */
#define SAMP   (NB/2)                       /* 4096 complex samples per heap = NCHAN*TPKT             */
            ...
            if(p0){
                ...
                for(int st=off; st+ULA_PPB<=npkts; st+=ULA_PPB){
                    ...
                    for(int j=0;j<ULA_PPB;++j){
                        ...
                        ++seen; }
                    if(seen<NELEM){imissed++;continue;}   /* incomplete snapshot -> skip */
