#!/usr/bin/env bash
# Run the LINE-RATE analytic-sky TX (tx_fp8_host) on the transmitter, inside the daqiri:local container.
# Streams the star + orbiting-planet sky from HOST memory at ~380 GbE -- bypassing the ~145 G single-GPU
# GPUDirect cap (run_tx.sh) -- by pre-filling the host TX buffers with the sky once, then re-sending them.
# Only stale payloads are rewritten as the planet moves, staggered so the send never stalls. Paced at 380
# via pacing_mbps in tx_host.yaml. Args pass through to tx_fp8_host (e.g. --rho 0.35 --torbit 3 --refresh-hz 8).
set -euo pipefail
...
exec docker run --rm --privileged --network host \
  ...
  -v "$DIR":/work -w /work \
  --entrypoint /work/tx_fp8_host "$IMG" tx_host.yaml "$@"
