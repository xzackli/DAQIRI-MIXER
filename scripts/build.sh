...
#   transmitter (A6000): tx_fp8 (GPUDirect sky, ~145 G) + tx_fp8_host (host-memory sky at line rate, ~380 G)
...
docker run --rm -v "$DIR":/work -w /work --entrypoint bash "$IMG" -lc "
  ...
  echo '[build] tx_fp8_host   (TX,  sm_86; host-mem, line rate)' ; nvcc -O3 -std=c++17 -arch=sm_86 src/tx_fp8_host.cu $DQ -o tx_fp8_host
