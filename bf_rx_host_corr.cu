/* bf_rx_host_corr.cu -- realtime int8 tensor-core COVARIANCE CORRELATOR on one RTX 5070, fed by a 400 GbE
 * F-engine stream. Per frequency channel: V = X*X^H (bf_cgemm.cuh), integrated, then imaged.
 ...
 *  compute: bf_cgemm_blk_t<true> accumulates V += X*X^H into a persistent int32 cube; k_flush_cube drains it
 *    to a float cube every FLUSH_N updates (no separate accumulate pass, no per-batch V buffer).
 *  publish @ fps: snapshot = float cube + live int32 partial; dV = V - Vprev; grid by baseline d=a-b -> 32x32;
 *    one batched cuFFT -> 128-channel (32x32) alias-free image cube -> /dev/shm/bf_corr32 (van Cittert-Zernike).
 * Build: nvcc -arch=sm_120a bf_rx_host_corr.cu -lcufft + daqiri libs (no ccglib, no DEFS).
 * Verified: star+orbiting planet, per-channel slope==c/127 (bf_spectest); ~380 G = ~98% line rate, 0-drop <~90%. */
#include <cuda_runtime.h>
#include <cuda_fp8.h>
#include <cufft.h>
#include "bf_cgemm.cuh"
#include <fcntl.h>
#include <sys/mman.h>
#include <unistd.h>
#include <atomic>
...
#define NB        (BF_NCH*BF_TPKT)           /* 8192 wire bytes per heap (planar fp8 re|im) */
...
#define KPCH      1024                        /* samples per channel per V-update (= HBATCH*BF_TPKT) */
#define MM        256                         /* M = N = BF_NANT */
...
static_assert(SAMP_PER_HEAP/BF_TPKT==NCUBE, "wire channels (SAMP_PER_HEAP/BF_TPKT) must equal NCUBE");
static_assert(HBATCH*BF_TPKT==KPCH, "KPCH must equal HBATCH*BF_TPKT");

/* pack: planar fp8 bb[heap][ant][re[SAMP]|im[SAMP]] -> int8 A=X and B=conj(X), BOTH [ch][ant][klocal]
 * (row-major, ld=KPCH). bf_cgemm reads B column-major, so this same layout IS conj(X)^T -- the transpose
 * is free and A/B share one coalesced index (no scattered transpose write). One thread per (ant, global
 * sample); the frequency channel IS the split-K slot c=sl/BF_TPKT, klocal=heap*BF_TPKT+t spans the KPCH
 * of the V-update. The (ant,heap,sl) -> (c,ant,klocal) map is a bijection, so every element is written once. */
__global__ void k_packAB(const uint8_t* __restrict__ bb, BfI8* __restrict__ Are, BfI8* __restrict__ Aim,
                         BfI8* __restrict__ Bre, BfI8* __restrict__ Bim){
    size_t i=(size_t)blockIdx.x*blockDim.x+threadIdx.x; if(i>=(size_t)BF_NANT*TOTAL_SAMP) return;
    int ant=i/TOTAL_SAMP; size_t s=i%TOTAL_SAMP;            /* global sample [0,TOTAL_SAMP) */
    int heap=s/SAMP_PER_HEAP, sl=s%SAMP_PER_HEAP, c=sl/BF_TPKT, t=sl%BF_TPKT, klocal=heap*BF_TPKT+t;
    size_t hb=(size_t)heap*BF_NANT*NB+(size_t)ant*NB;
    ...
    BfI8 re=(BfI8)(qr<-127?-127:(qr>127?127:qr)), im=(BfI8)(qi<-127?-127:(qi>127?127:qi));
    size_t a=(size_t)c*MM*KPCH+(size_t)ant*KPCH+klocal;     /* A=X, B=conj(X): [ch][ant][klocal], shared index */
    Are[a]=re; Aim[a]=im;                                   /* A = X         */
    Bre[a]=re; Bim[a]=(BfI8)(-im);                          /* B = conj(X)   (read col-major -> conj(X)^T) */
}
...
__global__ void k_grid_cube(const float* dV, cufftComplex* G){
    ...
    int a=i/BF_NANT,b=i%BF_NANT, dy=(((a>>4)-(b>>4))%NG+NG)%NG, dx=(((a&15)-(b&15))%NG+NG)%NG;
 ...
 * V += X*X^H directly into the int32 cube (Vire/Viim) every V-update -- this drains it to float every FLUSH_N
 * updates, before int32 can overflow (FLUSH_N*127*127*KPCH << 2^31). Vcube is [NCUBE][2][M][N] planar; the
...
/* publish snapshot = COMPLETE current visibility = flushed float cube + the not-yet-flushed int32 partial.
 * Without the int32 term the publish would only see the cube jump every FLUSH_N updates (staircase dV); adding
 * it makes dV smooth and current every frame. Benign race with the comp-stream RMW (same as the old snapshot). */
__global__ void k_snap_cube(float* __restrict__ Vsnap, const float* __restrict__ Vcube,
                            const int* __restrict__ Vire, const int* __restrict__ Viim){
    size_t i=(size_t)blockIdx.x*blockDim.x+threadIdx.x; if(i>=(size_t)NCUBE*MN) return;
    int ch=i/MN; size_t j=i%MN;
    Vsnap[(size_t)ch*VCH + j]      = Vcube[(size_t)ch*VCH + j]      + (float)Vire[i];
    Vsnap[(size_t)ch*VCH + MN + j] = Vcube[(size_t)ch*VCH + MN + j] + (float)Viim[i];
}

/* spectral-cube shm: NCUBE freq planes of NG x NG; viewer bf_peek32 selects a channel */
struct Corr32 { uint32_t magic, w, h, nch; volatile uint64_t write_seq; float img[NCUBE*NG*NG]; float vmax[NCUBE], vmin[NCUBE]; };
#define CORR_SHM "/bf_corr32"
...
static BfI8* g_Are[NBATCH]={0}; static BfI8* g_Aim[NBATCH]={0};   /* A = X         [ch][ant][klocal] (int8)   */
static BfI8* g_Bre[NBATCH]={0}; static BfI8* g_Bim[NBATCH]={0};   /* B = conj(X)^T [ch][klocal][ant] (int8)   */
static int*  g_Vire=nullptr; static int* g_Viim=nullptr;          /* SINGLE running int32 visibility cube [ch][M][N], flushed to g_Vcube */
#define FLUSH_N 16                                                /* flush int32 cube -> float every FLUSH_N V-updates (overflow-safe, smooth imaging) */
...
static void drain_thread(){ pin(g_drain_core);
    while(!g_stop){ daqiri::BurstParams* bu=nullptr;
        ...
        uint32_t seq0=__builtin_bswap32(*(const uint32_t*)(p0+BF_SEQ_BYTE)); int off=(int)((BF_NANT-(seq0%BF_NANT))%BF_NANT);
        if(off) g_misalign.fetch_add(1,std::memory_order_relaxed);
        for(int st=off; st+BF_PPB<=npkts; st+=BF_PPB){ const uint8_t* base=(const uint8_t*)daqiri::get_packet_ptr(bu,st);
            if(!base) continue;
            if(base<g_mr_base||base+(size_t)(BF_NANT-1)*MR_STRIDE+NB+BF_HDR_BYTES>g_mr_end){g_misalign.fetch_add(1,std::memory_order_relaxed);continue;}
...
static void gpu_thread(){ pin(g_gpu_core); CK(cudaSetDevice(g_device));
    ...
    dim3 gemm_grid((((MM/(BF_WT*BF_SM))*(MM/(BF_WT*BF_SN)))*32+255)/256, 1, NCUBE);  /* (8,1,128): one warp per SMxSN tile-block, z=channel */
    cudaEvent_t vacc_ev; CK(cudaEventCreateWithFlags(&vacc_ev,cudaEventDisableTiming)); /* serialize g_V += across comp streams */
    int packT=256, packG=(int)(((size_t)BF_NANT*TOTAL_SAMP+packT-1)/packT);
    ...
    while(true){
        ...
        CK(cudaMemcpy2DAsync(g_bb[cur]+(size_t)hc*BF_NANT*NB,NB,(const uint8_t*)g_slot[s].mr_ptr+BF_HDR_BYTES,MR_STRIDE,NB,BF_NANT,cudaMemcpyHostToDevice,copy_stream));
        ...
        if(++hc==(int)HBATCH){ if(!g_noflop){ int cm=(int)(bc%NCOMP);
                ...
                bf_cgemm_blk_t<true><<<gemm_grid,256,0,comp_stream[cm]>>>(MM,MM,KPCH,g_Are[cur],g_Aim[cur],g_Bre[cur],g_Bim[cur],g_Vire,g_Viim);
                if(++flushc>=FLUSH_N){ k_flush_cube<<<((size_t)NCUBE*MN+255)/256,256,0,comp_stream[cm]>>>(g_Vcube,g_Vire,g_Viim); flushc=0; }
...
int main(int argc,char**argv){
    if(argc<2){fprintf(stderr,"usage: %s <yaml> [--seconds N][--fps F][--device D][--noflop]\n",argv[0]);return 1;}
    ...
    g_port=daqiri::get_port_id(BF_RX_IFACE); if(g_port<0){fprintf(stderr,"no RX iface\n");return 1;}
    ...
    for(int b=0;b<NBATCH;++b){ CK(cudaMalloc(&g_bb[b],(size_t)HBATCH*BF_NANT*NB));
    ...
    float *g_Vsnap,*g_Vprev,*g_dV; cufftComplex *g_G,*g_Img;
    CK(cudaMalloc(&g_Vsnap,cubeF*4)); CK(cudaMalloc(&g_Vprev,cubeF*4)); CK(cudaMemset(g_Vprev,0,cubeF*4));
    ...
    while(!g_stop){
        ...
        /* snapshot complete V = float cube + live int32 partial (benign race w/ accumulate) -> dV -> grid -> FFT */
        k_snap_cube<<<(unsigned)(((size_t)NCUBE*MN+255)/256),256,0,pub>>>(g_Vsnap,g_Vcube,g_Vire,g_Viim);
        k_diff<<<(unsigned)((cubeF+255)/256),256,0,pub>>>(g_Vsnap,g_Vprev,g_dV,cubeF);
        ...
        CK(cudaMemcpyAsync(h,g_Img,(size_t)NCUBE*NG*NG*sizeof(cufftComplex),cudaMemcpyDeviceToHost,pub)); CK(cudaStreamSynchronize(pub));
